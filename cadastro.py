...
#atualizar notas
def atualizar_notas(id, nota1= None, nota2 = None):
    try:
        conexao = conectar()
        cursor = conexao.cursor()
        cursor.execute("SELECT nota1, nota2 FROM notas WHERE id_notas = %s", (id,))
        resultado = cursor.fetchall()
        if nota1 is None:
            nota1 = resultado[0]
        if nota2 is None:
            nota2 = resultado[1]
        cursor.execute("UPDATE notas SET nota1 = %s, nota2 = %s WHERE id_notas = %s", (nota1, nota2,  id))
        conexao.commit()
        print(f"Notas do ID {id} foram atualizadas!")   
    except pymysql.MySQLError as e:
        print(f"Erro ao atualizar as notas.")
    finally:
        if cursor:
            cursor.close()
        if conexao:
...
def deletar_notas(id):
    try:
        conexao = conectar()
        cursor = conexao.cursor()
        cursor.execute("DELETE FROM notas WHERE id_notas = %s", (id,))
        conexao.commit()
        print(f"Notas do ID {id} deletadas com sucesso!")
    except pymysql.MySQLError as e:
        print(f"Erro ao deletar as notas.")
    finally:
        if cursor:
            cursor.close()
        if conexao:
...
# Listar alunos por notas            
def alunos_por_notas():
    try:
        conexao = conectar()
        cursor = conexao.cursor()
        cursor.execute("SELECT a.nome_aluno, n.nota1, n.nota2, n.media, n.situacao FROM alunos as a JOIN notas as n ON a.id_aluno = n.id_aluno")
        resultado = cursor.fetchall()
        for alunos in resultado:
            print(f"[Nome: {alunos[0]}], [Nota 1: {alunos[1]}], [Nota 2: {alunos[2]}], [Média: {alunos[3]}], [Situação: {alunos[4]}]")
        return resultado
    except pymysql.MySQLError as e:
...
def buscar_aluno_por_nome(nome_aluno):
    try:
        conexao = conectar()
        cursor = conexao.cursor()
        cursor.execute("SELECT * FROM alunos WHERE nome_aluno = %s", (nome_aluno,))
        resultado = cursor.fetchall()
        for alunos in resultado:
            print(f"[ID: {alunos[0]}], [ Nome: {alunos[1]}], [ Email: {alunos[2]}], [ Telefone: {alunos[3]}], [ CPF: {alunos[4]}], [ Sexo: {alunos[5]}], [ Data de Nascimento: {alunos[6]}], [ Turma: {alunos[7]}]")
            return resultado
